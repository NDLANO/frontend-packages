...
import { BreadCrumbs, SearchResultLead, SearchResultLink, SearchBlockWrapper } from './Styles';
...
class SearchBlock extends Component {
    ...
    doSearch() {
        const query = `*[_type == "designmanualDocuments" && _id != "${this.props.currentPageId}" && title match "${this.props.searchFor}*"]{title, lead, _id, "root": root->{title}, "parent": parent->{title}}[0...11]`;
    ...
    render() {
        const links = {};
        if (window.location.origin === 'http://localhost:6006' || window.location.origin === 'https://designmanual.ndla.sh') {
