...
The `consumeCss` prop forces panda to convert the css object to a class name string. This mostly covers cases when you use `asChild` to merge a styled component with an underlying component that does not suppport python.
...
By providing the `baseComponent` option, panda more or less considers this component to be a string component. If you render the component normally, the `__cva__` property is auto-converted to a string. If you `asChild` the component, the css prop is forwarded to the child. If you're `asChild`-ing onto a non-panda component, you have to include the `consumeCss` prop.
