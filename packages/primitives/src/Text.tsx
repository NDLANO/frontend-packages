...
import type { ColorToken, FontWeightToken, TextStyleValue } from "@ndla/styled-system/types";
...
const StyledP = styled(ark.p, { base: { textStyle: "body.medium" } }, { baseComponent: true });
...
export const Text = forwardRef<HTMLParagraphElement, TextComponentProps>(
  ({ textStyle, fontWeight, color, srOnly, css: cssProp, ...rest }, ref) => (
    <StyledP css={css.raw({ textStyle, fontWeight, color, srOnly }, cssProp)} ref={ref} {...rest} />
  ),
);

const StyledH1 = styled(ark.h1, { base: { textStyle: "heading.medium" } }, { baseComponent: true });
