export const setCookie = (cookieName: string, cookieValue: string, removeCookie: boolean, lax?: boolean) => {
  let expires;
  if (removeCookie) {
  ...
  document.cookie = `${cookieName}=${cookieValue}; ${expires}; SameSite=${lax ? 'Lax' : 'Strict'}; path=/`;
};
