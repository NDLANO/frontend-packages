...
const rootDir = path.resolve(__dirname, '..', 'packages', 'ndla-icons');
const attrs = ['xlink:href', 'id'];

const copyright = fs.readFileSync(path.resolve(__dirname, '..', 'COPYRIGHT'), 'utf-8');
...
${exportsString}\n`;
    ...
    const fileName = path.resolve(rootDir, 'src', folder, 'index.ts');
    fs.writeFileSync(fileName, formatedIconsModule, 'utf-8');
    console.log(`${chalk.green(`CREATED`)} ${chalk.dim(path.resolve(rootDir, 'src'))}${chalk.bold(`${folder}/index.ts`)}`);
  });
}
    ...
    const iconsModule = `
{
  ...
  "jsnext:main": "../es${folder}/index.js"
    ...
    const fileName = path.resolve(rootDir, folder, 'package.json');
    ensureDirectoryExistence(fileName);
    fs.writeFileSync(fileName, iconsModule, 'utf-8');

    console.log(`${chalk.green(`CREATED`)} ${chalk.dim(path.resolve(rootDir))}${chalk.bold(`${folder}/package.json`)}`);
  });
}
...
function deleteComponents() {
  ...
  const getDirectories = (source) =>
    fs
      ...
      .filter(isDirectory);
  getDirectories(path.resolve(rootDir, 'src')).forEach((directory) => {
...
function createComponents() {
  ...
  glob(`${rootDir}/svg/*/*.svg`, (err, icons) => {
    ...
    icons.forEach((iconPath) => {
      ...
      const folder = iconPath.replace(path.resolve(`${rootDir}/`, 'svg'), '').replace(`/${path.basename(iconPath)}`, '');
      const name = capitalize(camelcase(id));
      const componentPath = path.resolve(folder, `${name}.tsx`);
      ...
      if (component) {
        const dest = path.resolve(rootDir, 'src', componentPath);
        ensureDirectoryExistence(dest);
        fs.writeFileSync(dest, component, 'utf-8');
        console.log(`${chalk.green(`CREATED`)} ${chalk.dim(path.resolve(rootDir, 'src'))}${chalk.bold(componentPath)}`);
      } else {
        failed.push(path.resolve(rootDir, 'src', componentPath));
        console.log(`${chalk.red(`Failed`)} ${chalk.dim(path.resolve(rootDir, 'src'))}${chalk.bold(componentPath)}`);
