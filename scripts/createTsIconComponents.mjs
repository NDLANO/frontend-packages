...
const licensePattern = /\sdata-license=(['"])(.*?)\1/;
const sourcePattern = /\sdata-source=(['"])(.*?)\1/;
const licensePatternGlobal = /\sdata-license=(['"])(.*?)\1/g;
const sourcePatternGlobal = /\sdata-source=(['"])(.*?)\1/g;
const yearPattern = /(\d{4})-/g; // This finds the year so we can replace it with current year in the .tsx file

const getSvgMetadata = (file, filename) => {
  const license = file.match(licensePattern)?.[2];
  const source = file.match(sourcePattern)?.[2];
  ...
  return {
    license,
    source,
    sanitizedFile: file.replace(licensePatternGlobal, "").replace(sourcePatternGlobal, ""),
  };
};

const getMetadataComment = ({ license, source }) => `// Source: ${source}\n// License: ${license}`;
...
const filePromises = allFiles.map(async ({ filename, file, ctime, mtime }) => {
  ...
  const { license, source, sanitizedFile } = getSvgMetadata(file, filename);
  const headerNotice = `${autoNotice}\n${getMetadataComment({ license, source })}`;
  const component = await transform(
    sanitizedFile,
    {
      ...
      template: (variables, { tpl }) => {
        return tpl`
            ${copyright.replace(yearPattern, `${ctime.getFullYear()}-`) + "\u000A"}
